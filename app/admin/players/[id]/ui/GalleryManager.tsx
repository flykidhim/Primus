...
export default function GalleryManager({ playerId, photos }: { playerId: string; photos: any[] }) {
  ...
  async function add() {
    if(!url) return
    const p = await fetch(`/api/admin/players/${playerId}/images`, {
  ...
  async function remove(imageId: string) {
    ...
    const res = await fetch(`/api/admin/players/${playerId}/images/${imageId}`, { method:'DELETE' })
    ...
    <div className="glass rounded-2xl p-4">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        <div className="md:col-span-2 grid grid-cols-2 sm:grid-cols-3 gap-3">
          {list.map(pi=>(
            <div key={pi.id} className="relative rounded-xl overflow-hidden border border-white/10">
              <img src={pi.url} alt={pi.alt || ''} className="w-full h-40 object-cover" />
              <div className="p-2 text-xs flex items-center justify-between bg-black/40">
                <span>sort: {pi.sort}</span>
                <button onClick={()=>remove(pi.id)} className="px-2 py-1 rounded bg-white/10 hover:bg-white/20">Remove</button>
              </div>
            </div>
          ))}
        </div>
        <div className="grid gap-2">
          ...
          <p className="text-xs text-white/60">Tip: use <code>placehold.co</code> now; swap to CDN later.</p>
        </div>
      </div>
    </div>
  )
